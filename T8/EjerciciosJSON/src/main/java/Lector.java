...
public void esquiposEspanioles(){
        String urlespania = "https://www.thesportsdb.com/api/v1/json/3/search_all_teams.php?s=Soccer&c=Spain";
    BufferedReader bufferedReader = null;
    try {
        URL url =new URL(urlespania) ;
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
        bufferedReader = new BufferedReader(new InputStreamReader(connection.getInputStream()));
        String lectura = bufferedReader.readLine();
        JSONObject objeto = new JSONObject(lectura);
        JSONArray arrayEquipo = objeto.getJSONArray("teams");
        for (int i = 0; i <arrayEquipo.length(); i++) {
            JSONObject equipo = arrayEquipo.getJSONObject(i);
