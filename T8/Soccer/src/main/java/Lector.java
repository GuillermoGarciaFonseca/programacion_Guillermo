...
public class Lector {
    ...
    public void obtenerClasificacion(){
        ...
        try {
            ...
            for (int i = 0; i <arrayTabla.length(); i++) {
                ...
                listaEquipos.add(new Equipo(nombre,posicion));
            }
        } catch (MalformedURLException e) {
            //throw new RuntimeException(e);
        } catch (IOException e) {
    ...
    public void obtenerEquipos(String pais) {
        String urlEquipos = "https://www.thesportsdb.com/api/v1/json/3/search_all_teams.php?s=Soccer&c=" + pais;
        BufferedReader bufferedReader = null;
        try {
            URL url = new URL(urlEquipos);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            bufferedReader = new BufferedReader(new InputStreamReader(connection.getInputStream()));
            String lectura = bufferedReader.readLine();
            JSONObject objeto = new JSONObject(lectura);
            JSONArray arrayEquipos = objeto.getJSONArray("teams");
            for (int i = 0; i < arrayEquipos.length(); i++) {
                JSONObject equipo = arrayEquipos.getJSONObject(i);
                String nombreEquipo = equipo.getString("strTeam");
                String idEquipo = equipo.getString("idTeam");
                listaEquipos.add(new Equipo(nombreEquipo,idEquipo));
            }
        } catch (MalformedURLException e) {
            //throw new RuntimeException(e);
        } catch (IOException e) {
    ...
    public void obtenerUltimosPartidosEquipo(String idEquipo) {
        String urlPartidos = "https://www.thesportsdb.com/api/v1/json/3/eventslast.php?id=" + idEquipo;
        BufferedReader bufferedReader = null;
        try {
            URL url = new URL(urlPartidos);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            bufferedReader = new BufferedReader(new InputStreamReader(connection.getInputStream()));
            String lectura = bufferedReader.readLine();
            JSONObject objeto = new JSONObject(lectura);
            JSONArray arrayPartidos = objeto.getJSONArray("results");
            for (int i = 0; i < arrayPartidos.length(); i++) {
                JSONObject partido = arrayPartidos.getJSONObject(i);
                String fecha = partido.getString("dateEvent");
                String local = partido.getString("strHomeTeam");
                String visitante = partido.getString("strAwayTeam");
                System.out.printf("Fecha: %s, Local: %s, Visitante: %s%n", fecha, local, visitante);
            }
        } catch (MalformedURLException e) {
            throw new RuntimeException(e);
        } catch (IOException e) {
